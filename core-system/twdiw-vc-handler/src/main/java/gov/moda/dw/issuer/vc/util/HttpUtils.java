...
public class HttpUtils {
    ...
    // 準備白名單 WHITE_MAP
    private static final Map<String, String> WHITE_MAP = new ConcurrentHashMap<>();
    ...
    // 靜態 PreloadSetting 參考
    private static PreloadSetting preloadSetting;

    private static String extractHostname(String url){
        try{
            // 從目標 url 取 hostname (domain name 或 ip)
            URL u = new URL(url);
    ...
    private static void initializeWhiteMap() throws VcException {
        // 從 Spring 容器內取 preloadSetting 這隻 bean 出來
        preloadSetting = SpringContextHolder.getBean(gov.moda.dw.issuer.vc.vo.PreloadSetting.class);

        // 將 preloadSetting 內所有會用來連線的 URL 都先建到 WHITE_MAP 內: (key, value) = (hostname, url)
        // ex: ("localhost", "https://localhost:8443/exampleApp/api/test")
        try {
            // urlVpValidation
            String urlVpValidation = preloadSetting.getUrlVpValidation();
            String hostnameVpValidation = extractHostname(urlVpValidation);
            if (urlVpValidation != null && !urlVpValidation.isEmpty() && hostnameVpValidation != null
                    && !hostnameVpValidation.isEmpty()) {
                WHITE_MAP.put(hostnameVpValidation, urlVpValidation);
            }

            // urlDemoGetHolderData
            String urlDemoGetHolderData = preloadSetting.getUrlDemoGetHolderData();
            String hostnameDemoGetHolderData = extractHostname(urlDemoGetHolderData);
            if (urlDemoGetHolderData != null && !urlDemoGetHolderData.isEmpty() && hostnameDemoGetHolderData != null
                    && !hostnameDemoGetHolderData.isEmpty()) {
                WHITE_MAP.put(hostnameDemoGetHolderData, urlDemoGetHolderData);
            }

            // urlFrontendCreateDid
            String urlFrontendCreateDid = preloadSetting.getUrlFrontendCreateDid();
            String hostnameFrontendCreateDid = extractHostname(urlFrontendCreateDid);
            if (urlFrontendCreateDid != null && !urlFrontendCreateDid.isEmpty() && hostnameFrontendCreateDid != null
                    && !hostnameFrontendCreateDid.isEmpty()) {
                WHITE_MAP.put(hostnameFrontendCreateDid, urlFrontendCreateDid);
            }

            // urlFrontendGenerateDid
            String urlFrontendGenerateDid = preloadSetting.getUrlFrontendGenerateDid();
            String hostnameFrontendGenerateDid = extractHostname(urlFrontendGenerateDid);
            if (urlFrontendGenerateDid != null && !urlFrontendGenerateDid.isEmpty()
                    && hostnameFrontendGenerateDid != null && !hostnameFrontendGenerateDid.isEmpty()) {
                WHITE_MAP.put(hostnameFrontendGenerateDid, urlFrontendGenerateDid);
            }

            // urlFrontendGetIssuerInfoDid
            String urlFrontendGetIssuerInfoDid = preloadSetting.getUrlFrontendGetIssuerInfoDid();
            String hostnameFrontendGetIssuerInfoDid = extractHostname(urlFrontendGetIssuerInfoDid);
            if (urlFrontendGetIssuerInfoDid != null && !urlFrontendGetIssuerInfoDid.isEmpty()
                    && hostnameFrontendGetIssuerInfoDid != null && !hostnameFrontendGetIssuerInfoDid.isEmpty()) {
                WHITE_MAP.put(hostnameFrontendGetIssuerInfoDid, urlFrontendGetIssuerInfoDid);
            }

            // urlFrontendRegisterDid
            String urlFrontendRegisterDid = preloadSetting.getUrlFrontendRegisterDid();
            String hostnameFrontendRegisterDid = extractHostname(urlFrontendRegisterDid);
            if (urlFrontendRegisterDid != null && !urlFrontendRegisterDid.isEmpty()
                    && hostnameFrontendRegisterDid != null && !hostnameFrontendRegisterDid.isEmpty()) {
                WHITE_MAP.put(hostnameFrontendRegisterDid, urlFrontendRegisterDid);
            }

            // urlFrontendReviewDid
            String urlFrontendReviewDid = preloadSetting.getUrlFrontendReviewDid();
            String hostnameFrontendReviewDid = extractHostname(urlFrontendReviewDid);
            if (urlFrontendReviewDid != null && !urlFrontendReviewDid.isEmpty() && hostnameFrontendReviewDid != null
                    && !hostnameFrontendReviewDid.isEmpty()) {
                WHITE_MAP.put(hostnameFrontendReviewDid, urlFrontendReviewDid);
            }

            // urlVcBasePath
            String urlVcBasePath = preloadSetting.getUrlVcBasePath();
            String hostnameVcBasePath = extractHostname(urlVcBasePath);
            if (urlVcBasePath != null && !urlVcBasePath.isEmpty() && hostnameVcBasePath != null
                    && !hostnameVcBasePath.isEmpty()) {
                WHITE_MAP.put(hostnameVcBasePath, urlVcBasePath);
            }

            // urlOidvciQrcode
            String urlOidvciQrcode = preloadSetting.getUrlOidvciQrcode();
            String hostnameOidvciQrcode = extractHostname(urlOidvciQrcode);
            if (urlOidvciQrcode != null && !urlOidvciQrcode.isEmpty() && hostnameOidvciQrcode != null
                    && !hostnameOidvciQrcode.isEmpty()) {
                WHITE_MAP.put(hostnameOidvciQrcode, urlOidvciQrcode);
            }
        } catch (Exception e) {
            LOGGER.error(e.getMessage(), e);
            String errorMessage = "fail to initializeWhiteMap";
            throw new VcException(VcException.ERR_SYS_GET_PRELOAD_SETTING_ERROR, errorMessage);
        }
    }

    private String checkTargetUrl(String url){
        String result = null;
        ...
        try{
            ...
            // 使用 hostname 到白名單 WHITE_MAP 裡面去找
            if(WHITE_MAP != null && !WHITE_MAP.isEmpty() && WHITE_MAP.containsKey(hostname)){
//                result = WHITE_MAP.get(hostname);
                result = url; // 改動(不太懂設計邏輯): 直接使用原 url 去連線，白名單內的 url 只是用來驗證 hostname 是否被允許連線
            }
        } catch (Exception e) {
            LOGGER.error(e.getMessage(), e);
        }
    ...
    public HttpUtils(Builder builder) {
        ...
        // 印出 HttpUtils 設定
        if (enableLog) {
            LOGGER.info("=== HttpUtils 設定 ===");
            LOGGER.info("URL: {}", url);
            LOGGER.info("Timeout: {}ms", maxConnectTimeout);
            LOGGER.info("Retry Times: {}", maxRetryTimes);
            LOGGER.info("Enable Log: {}", enableLog);

            if (useProxy) {
                LOGGER.info("Proxy 設定: {}://{}:{}", proxyServerProtocol, proxyServerAddress, proxyServerPort);
            } else {
                LOGGER.info("Proxy 設定: 未使用 (直接連線)");
            }
            LOGGER.info("====================");
        }
    }
    ...
    public Tuple.Pair<Integer, String> get() {
        ...
        try {
            ...
            if (enableLog) {
                ...
                LOGGER.info("\nHTTP RESP {} ({})\nHTTP RESP {} {}", symbol, statusCode, symbol, content);
            }
...
//    }
    ...
    private Tuple.Pair<Integer, String> post(HttpEntity entity, String contentType) {
        ...
        try {
            // 將 url 先丟去 checkTargetUrl()，裡面要經過 WHITE_MAP 白名單檢查
            String checkedUrl = checkTargetUrl(url);
            if(checkedUrl == null){
                throw new Exception("invalid url for http post (not in white list)");
            }

            HttpPost httpPost = new HttpPost(checkedUrl);
...
//                LOGGER.info("\nHTTP REQ {} {}\nHTTP REQ {} {}", symbol, httpPost.getRequestLine(), symbol, EntityUtils.toString(entity));
            }
    ...
    public static class Builder {
        ...
        public Builder() {
            ...
            usePreloadSetting();
                ...
                /**
         * 使用 PreloadSetting 的設定值
         *
         * @return Builder instance
         */
        public Builder usePreloadSetting() {
            try {
                // 透過 SpringContextHolder 取得 PreloadSetting
                gov.moda.dw.issuer.vc.vo.PreloadSetting preloadSetting =
                    SpringContextHolder.getBean(gov.moda.dw.issuer.vc.vo.PreloadSetting.class);

                if (preloadSetting != null) {
                    this.maxConnectTimeout = preloadSetting.getHttpMaxConnectTimeout();
                    this.maxRetryTimes = preloadSetting.getHttpMaxRetryTimes();
                    this.enableLog = preloadSetting.isHttpEnableLog();

                    // 使用 proxy 設定
                    if (preloadSetting.isHttpProxyEnable()) {
                        this.useProxy = true;
                        this.proxyServerAddress = preloadSetting.getHttpProxyUrl();
                        this.proxyServerPort = preloadSetting.getHttpProxyPort();
                        this.proxyServerProtocol = preloadSetting.getHttpProxyProtocol();
                    }
                }
            } catch (Exception e) {
                // 如果無法取得 PreloadSetting，使用預設值
                LOGGER.warn("無法取得 PreloadSetting，使用預設值: {}", e.getMessage());
            }
            return this;
        }
